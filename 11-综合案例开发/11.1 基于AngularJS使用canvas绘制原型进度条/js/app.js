angular.module('angular.directive-load-progress', [])
        ...
        return {
            ...
            compile: function(tplele, tplattr, transclude) {
                if (tplele.length === 1) {
                    ...
                    var iccolor = node.getAttribute('progress-inner-circle-background-color') || '#666';
...
angular.module('progress', ['angular.directive-load-progress'])
    .controller('ctrl_progress', ['$scope', '$interval', function($scope, $interval) {
        ...
        /*
            $warch方法：
            第一个参数：angular表达式或函数字符串
            第二个参数：发生变化时被调用的函数
            第三个参数：可选的布尔值，检查被监测的对象的每个属性是否发生变化
        * */
        $scope.$watch('ProgressValues', function(newValue) {
